<script>
	...
	import { appSession, loading, personalBio, scrollPosition } from "$lib/appStore";
    ...
    const handleComment = () => {
      if(!$appSession) {return goto('/auth/signin')}
      loading.set(true)
...
<div class="fixed bottom-0 lg:static w-full">
    {#if showComment}
        <div class="">
    ...
    <div class="navbar px-4 sticky top-0 space-x-6 items-center flex justify-between rounded-lg bg-base-300 min-h-[1rem] h-[4rem] ">
        ...
        {#if $singleLens.published }
            <div class="flex relative items-center -space-x-3">
                <button on:click={handleShowComment} class={`btn ${showComment ? ' text-[#35c5ec] ' : ' '} relative hover:z-10 cursor-pointer`} ><MessageCircle class='h-5 w-5' /></button>
                <button class="btn relative hover:z-10 cursor-pointer" on:click={handleLike}><Heart class={`h-5 w-5 ${liked ? 'text-[#35c5ec]' : 'stroke-slate-400'}`} /></button>
            </div>
        {/if}
    </div>  
</div>
